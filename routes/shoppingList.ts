...
import Author from '../models/user'; // User model
...
// get all
router.get('/', auth, query('token').isString(), async (req: Request, res: Response) => {
  const { token } = req.query;

  const { user_id, email } = dataFromToken(token as string);
...
// get single
router.get('/single', query('id').isMongoId(), async (req: Request, res: Response) => {
  const { id } = req.query;
  // console.log(req)
...
router.post('/add', auth, body('recipes').isArray(), body('token').isString(), async (req: Request, res: Response) => {
  ...
  if (recipes.some((recipe: string) => recipe.length !== 24)) {
    throw new Error('shoppinglist add - bad recipes id ');
  }

  const awaitForRecipes = recipes.map(async (id: string) => {
  ...
  allIngredients.forEach((ingredients) => {
    ingredients.forEach((el: any) => allIngredientsArray.push(el));
  });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const shoppingList1 = await ShoppingList.findById(new mongoose.Types.ObjectId(recipe1));
    const shoppingList2 = await ShoppingList.findById(new mongoose.Types.ObjectId(recipe2));
...
// Edit shopping list
router.put('/edit', body('id').isMongoId(), body('recipes').isArray(), body('author').isMongoId(), async (req: Request, res: Response) => {
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const updatedIngredientIndex = ingredients.findIndex((ingredient: any) => ingredient.name === ingredientName);
